...
FONT_PATH = (
    "C:/Windows/Fonts/consola.ttf" if os.path.exists("C:/Windows/Fonts/consola.ttf") else None
)
FONT_BOLD_PATH = (
    "C:/Windows/Fonts/consolab.ttf"
    if os.path.exists("C:/Windows/Fonts/consolab.ttf")
    else FONT_PATH
)
...
def render_tui_screen(
    ...
    idx_str = (
        f"Index: {indexed_count} video ({indexed_count * 49} frames)"
        if indexed_count > 0
        else "Index: Ready (0 videos)"
    )
    draw.text(
        (x_text, 72),
        f"Model: MobileCLIP2-S0 (CPU)   Storage: sqlite-vec   {idx_str}",
        fill=TEXT_MUTED,
        font=font_small,
    )
    ...
    if show_results:
        ...
        if is_sel_1:
            ...
            draw.rectangle(
                [28, y_content - 4, 31, y_content + 64], fill=BLUE_PRIMARY
            )  # Left accent bar

        draw.text(
            (38, y_content), "#1   00:00:37.0 -> 00:01:06.0", fill=BLUE_PRIMARY, font=font_bold
        )
        draw.text(
            (38, y_content + 22),
            "File: cctv-people-demo.webm   Peak: 00:00:48.0",
            fill=TEXT_MUTED,
            font=font_small,
        )
        draw.text(
            (38, y_content + 42),
            "=> Action: Type /open 1 to play moment in VLC",
            fill=BLUE_PRIMARY,
            font=font_small,
        )
        ...
        draw.text(
            (38, y_content + 22),
            "File: cctv-people-demo.webm   Peak: 00:00:11.0",
            fill=TEXT_MUTED,
            font=font_small,
        )
        ...
        draw.text(
            (38, y_content + 22),
            "File: cctv-people-demo.webm   Peak: 00:00:28.0",
            fill=TEXT_MUTED,
            font=font_small,
        )
    ...
    draw.text(
        (28, 532),
        "[Enter] Submit  ·  /index <dir>  ·  /open <id>  ·  /exit",
        fill=TEXT_DIM,
        font=font_small,
    )
...
def main() -> None:
    ...
    frames.append(render_shell_screen([("PS C:\\Users\\Felix\\videos> ", BLUE_PRIMARY)]))
    durations.append(400)

    launch_cmd = "amon-hen"
    for i in range(1, len(launch_cmd) + 1, 2):
        frames.append(
            render_shell_screen([(f"PS C:\\Users\\Felix\\videos> {launch_cmd[:i]}", TEXT_WHITE)])
        )
        durations.append(40)
    ...
    print(
        f"Successfully generated ultra-clean demo ({len(frames)} frames, {out_path.stat().st_size // 1024} KB)"
    )
